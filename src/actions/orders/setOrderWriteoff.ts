  ...
  return action('setOrderWriteoff', 'SQL', {
    ...
    query: `
      WITH lck AS (
        ...
        SELECT {{params.order_id}}::bigint, ({{params.amount}})::numeric, TRIM({{params.reason}}::text), {{params.actor}}::text
        FROM cap
        ...
        SELECT 'order_writeoffs', ins.id::text, 'writeoff_set', {{params.actor}}::text,
               jsonb_build_object('order_id', {{params.order_id}}::bigint, 'amount_usd', ins.amount_usd, 'reason', TRIM({{params.reason}}::text))
        FROM ins
        RETURNING row_pk
      ), audit_clear AS (
        INSERT INTO audit_log (table_name, row_pk, action, actor, new_data)
        SELECT 'order_writeoffs', del.id::text, 'writeoff_cleared', {{params.actor}}::text,
