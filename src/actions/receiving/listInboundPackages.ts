...
/**
 * The SELECTED CAMPAIGN's packages with address label, vendor code, and
 * item lines as jsonb — the single feed for the dashboard, filters, and
 * heads-up banner. Received packages stay listed (they are the inventory
 * story); the UI separates them by received_at. Campaign-scoped since the
 * Flash Buy's first box appeared inside MB5's receiving view.
 */
function listInboundPackages() {
  return action('listInboundPackages', 'SQL', {
    ...
    query: `
      ...
      SELECT p.id, p.group_buy_id, p.receive_address_id, ra.label AS address_label, ra.active AS address_active,
