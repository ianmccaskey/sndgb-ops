-- Address-group semantics in BOTH transfer-creating fns (full DDL —
-- replayable, per the Codex round-1 finding on 1786476900 which had
-- documented these changes only in comments). Three changes vs the
-- prior live versions, everything else identical:
--  (1) after the row-locked v_addr load: refuse a member address
--      (transfer_origin_id NOT NULL) as the transfer source — transfers
--      ship only FROM a group origin, so reservations and the 42004
--      lock keep keying on the origin's id;
--  (2) the on-hand check aggregates v_address_inventory over every
--      address whose COALESCE(transfer_origin_id, id) = the origin;
--  (3) the reservation LATERAL resolves through the SAME group mapping,
--      so an unfinished draft created from a member address
--      (pre-grouping, or after a regroup) still holds stock against the
--      origin — grouped stock cannot be double-reserved across old and
--      new draft shapes.
CREATE OR REPLACE FUNCTION public.create_transfer_draft(p_from_address_id bigint, p_destination_label text, p_destination jsonb, p_parcel jsonb, p_carrier text, p_servicelevel text, p_rate_amount text, p_rate_currency text, p_shippo_rate_id text, p_items jsonb, p_allow_over_onhand boolean, p_expected_from jsonb, p_destination_id bigint, p_expected_destination jsonb, p_note text, p_actor text, p_direct_order_item_id bigint, p_group_buy_id bigint)
