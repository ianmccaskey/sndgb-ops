-- Transfer draft creation moves into a VOLATILE plpgsql function so the
-- availability check and the insert are SERIALIZED per ship-from address:
-- pg_advisory_xact_lock(42004, address) makes concurrent creators queue,
-- and READ COMMITTED gives each statement inside the function a FRESH
-- snapshot, so the over-check always sees the draft a competitor just
-- committed (the single-statement CTE version could not — both could read
-- the same pre-insert state). Same pattern as commit_stock_plan.
-- Also enforced here: active + content-matched ship-from (expected_from),
-- active + content-matched SAVED destination (expected_destination, NULL
-- for custom addresses which are the explicit bypass), all-valid item
-- lines, and the state-aware reservation windows. Returns (id, claimed_at)
-- like the CTE version; zero rows = refused.
...
BEGIN
  -- one creator at a time per ship-from address
  ...
  -- the quote was priced for these exact contents
  ...
  -- SAVED destination freshness: refuse if the row was edited or archived
  -- since rates were fetched (custom destinations pass NULL = bypass)
  ...
  -- effective availability = on-hand minus live reservations; this
  -- statement's snapshot is taken AFTER the advisory lock, so a competing
  -- draft that just committed is visible here
