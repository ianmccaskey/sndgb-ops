-- Review hardening on 1786473400-3600, two defects:
--
-- (1) The ship-from CAS was not serialized: both shipment fns read the
--     receive_addresses row WITHOUT a lock, so under READ COMMITTED a
--     concurrent address edit between the content-CAS check and the
--     insert could leave a stale origin snapshot on a (to-be-)purchased
--     label. Both reads now take FOR UPDATE, holding the row until
--     commit so the compared content IS the snapshotted content.
--
-- (2) shipments_manual_tracking_uniq was a GLOBAL unique index on the
--     manual tracking fingerprint — contradicting the documented 120-day
--     recycling policy (a legitimately recycled USPS number would be
--     blocked forever with no override). Dropped. The race it backstopped
--     (two concurrent manual records of one label) is now closed exactly:
--     create_manual_shipment takes a transaction-scoped advisory lock on
--     the normalized tracking fingerprint BEFORE the 120-day window
--     check, so concurrent duplicates serialize and the second sees the
--     first's committed row; reuse outside the window stays legal.
--
-- Both fns re-created in full below — exactly what was executed live.
DROP INDEX shipments_manual_tracking_uniq;
...
BEGIN
  ...
  -- FOR UPDATE: the CAS-compared content must BE the snapshotted content —
  -- an address edit committing between check and insert must wait or win
...
BEGIN
  ...
  IF p_ship_from_address_id IS NOT NULL THEN
    -- FOR UPDATE: same CAS-equals-snapshot guarantee as the draft fn
    SELECT * INTO v_addr FROM receive_addresses ra
    WHERE ra.id = p_ship_from_address_id AND ra.active
    FOR UPDATE;
    IF NOT FOUND THEN RETURN; END IF;
  ...
  -- race backstop replacing the dropped global unique index: concurrent
  -- manual records of the same fingerprint serialize here, so the second
  -- transaction's window check below sees the first's committed row;
  -- reuse OUTSIDE the 120-day window stays legal (the index could not
  -- express that)
  PERFORM pg_advisory_xact_lock(hashtextextended('ship_track_' || regexp_replace(v_tracking, '[^A-Z0-9]', '', 'g'), 42006));
