-- Comped (free) products on individual orders. Some customers are given a
-- product free; the upstream order still bills it, so those orders read
-- 'short' forever and P&L overstates expected revenue. comp_qty marks how
-- many units of an order line are free:
--
--  - reconciliation compares received against DUE (billed - comp value),
--    so a correctly-paid order with a comped line reads 'matched';
--  - billed_usd stays the upstream figure for transparency; comp_usd and
--    due_usd are exposed alongside it;
--  - P&L gets a comps_usd deduction (the give-away value at GB price) so
--    total revenue and net profit reflect money actually expected.
--
-- comp_qty lives on order_items (not a separate table) because the import
-- upsert only ever updates qty/unit_price on conflict and the prune only
-- removes lines deleted upstream — comps survive re-imports. LEAST(comp,
-- qty) guards the case where upstream later reduces qty below the comp.

ALTER TABLE order_items ADD COLUMN comp_qty NUMERIC(10,2) NOT NULL DEFAULT 0;
ALTER TABLE order_items ADD COLUMN comp_reason TEXT;
ALTER TABLE order_items ADD CONSTRAINT order_items_comp_qty_nonneg CHECK (comp_qty >= 0);
...
SELECT
  ...
  o.total_usd - COALESCE(cp.comp_usd, 0) AS due_usd,
  COALESCE(pv.verified_usd, 0) AS received_usd,
  ov.override_usd,
  COALESCE(ov.override_usd, COALESCE(pv.verified_usd, 0)) AS effective_received_usd,
  (o.total_usd - COALESCE(cp.comp_usd, 0)) - COALESCE(ov.override_usd, COALESCE(pv.verified_usd, 0)) AS diff_usd,
  COALESCE(pp.pending_count, 0) AS pending_payment_count,
  CASE
    WHEN COALESCE(pv.verified_usd, 0) = 0 AND ov.override_usd IS NULL
         AND (o.total_usd - COALESCE(cp.comp_usd, 0)) > gb.reconcile_tolerance_usd THEN 'awaiting'
    WHEN ABS((o.total_usd - COALESCE(cp.comp_usd, 0)) - COALESCE(ov.override_usd, COALESCE(pv.verified_usd, 0))) <= gb.reconcile_tolerance_usd THEN 'matched'
    WHEN (o.total_usd - COALESCE(cp.comp_usd, 0)) > COALESCE(ov.override_usd, COALESCE(pv.verified_usd, 0)) THEN 'short'
    ELSE 'over'
  END AS recon_status
FROM orders o
...
-- v_group_buy_pnl gains a comps_usd deduction: comped units still cost
-- vendor money (already inside product profit's cost side) but produce no
-- customer revenue, so both total revenue and net subtract the give-away.
...
SELECT
  ...
  COALESCE(prod.expected_revenue_usd, 0) + COALESCE(ord.admin_fees_usd, 0)
    ...
    - COALESCE(cmp.comps_usd, 0) AS total_revenue_usd,
  ...
  COALESCE(prod.product_profit_usd, 0) + COALESCE(ord.admin_fees_usd, 0)
    ...
    - COALESCE(cmp.comps_usd, 0) AS net_profit_usd
FROM group_buys gb
LEFT JOIN (
  SELECT group_buy_id,
