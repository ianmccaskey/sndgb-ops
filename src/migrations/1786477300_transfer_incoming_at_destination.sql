-- Transfers to one of OUR receive addresses materialize as an INCOMING
-- inbound package at the destination (Ian: "transfers should show as
-- incoming to the destination"): committed on creation (trackable) and
-- physically receivable on arrival. transfers.dest_receive_address_id
-- records the destination address (set from the 'ra_' picker); the
-- MANUAL fn creates the incoming package immediately (it has tracking),
-- the Shippo path creates it at FINALIZE (finalizeTransfer action —
-- source-controlled in src/actions). Creation is guarded against an
-- existing ACTIVE package on the same (carrier, tracking). Both create
-- fns gain a trailing p_dest_receive_address_id (signature change:
-- DROP + CREATE applied live 2026-08-31).
ALTER TABLE transfers ADD COLUMN dest_receive_address_id bigint REFERENCES receive_addresses(id);

DROP FUNCTION IF EXISTS public.create_transfer_draft(bigint, text, jsonb, jsonb, text, text, text, text, text, jsonb, boolean, jsonb, bigint, jsonb, text, text, bigint, bigint, bigint);
DROP FUNCTION IF EXISTS public.create_manual_transfer(bigint, text, jsonb, text, text, text, jsonb, boolean, jsonb, bigint, jsonb, text, text, bigint, bigint, bigint);
...
BEGIN
  ...
  INSERT INTO transfers (from_address_id, from_label, from_address, destination_label, destination, parcel,
                         ...
                         finalized_at, direct_order_item_id, direct_stamped_at, source_package_id, dest_receive_address_id)
  VALUES (v_addr.id, v_addr.label,
          jsonb_build_object('name', v_addr.name, 'street1', v_addr.street1, 'street2', v_addr.street2,
                             'city', v_addr.city, 'state', v_addr.state, 'zip', v_addr.zip,
                             'country', v_addr.country, 'phone', v_addr.phone, 'email', v_addr.email),
