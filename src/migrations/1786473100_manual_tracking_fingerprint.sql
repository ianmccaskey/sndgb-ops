-- Review hardening on 1786473000: the manual-label duplicate key must
-- be independent of OPERATOR FORMATTING. Receipts print tracking
-- numbers grouped ("9400 1000 0000 ...") and free-text carriers can be
-- aliased ("USPS" vs "US Postal"), so the raw (carrier, tracking) key
-- could miss a retype of the same physical label. Two changes:
--
-- (1) create_manual_transfer stores tracking with internal WHITESPACE
-- removed (whitespace is never part of a tracking number) on top of
-- UPPER(TRIM) — the stored value is the canonical compact form every
-- tracking API wants.
--
-- (2) The unique index is rebuilt on an alphanumeric FINGERPRINT of
-- the tracking alone: regexp_replace(UPPER(tracking), '[^A-Z0-9]', '')
-- — hyphens/spaces/dots collapse, and carrier is DROPPED from the key
-- entirely (a free-text carrier alias can therefore never bypass the
-- backstop; two different carriers sharing one tracking string is not
-- a real state — the fingerprint identifies the physical label).
-- Still scoped to manual rows (shippo_rate_id IS NULL). Preflight was
-- verified vacuous at apply time (zero manual rows exist).
...
BEGIN
  IF EXISTS (
    ...
    GROUP BY regexp_replace(UPPER(tracking_number), '[^A-Z0-9]', '', 'g')
    HAVING count(*) > 1
  ) THEN
    RAISE EXCEPTION 'duplicate manual tracking fingerprints exist — deduplicate before this migration';
  END IF;
END
$chk$;

CREATE UNIQUE INDEX transfers_manual_tracking_uniq
  ON transfers (regexp_replace(UPPER(tracking_number), '[^A-Z0-9]', '', 'g'))
  WHERE shippo_rate_id IS NULL AND tracking_number IS NOT NULL;

-- create_manual_transfer re-created (same signature, CREATE OR
-- REPLACE): tracking is normalized ONCE into v_tracking =
-- regexp_replace(UPPER(TRIM(...)), '\s', '', 'g') and that value is
-- used for the emptiness check, the INSERT, and both audit payloads.
-- Full text — exactly what was executed live (verified via
-- pg_get_functiondef containing v_tracking).
