-- Transfers can carry a link to ONE outstanding vendor-direct order line
-- (order_items.direct_ship): shipping a box from a receive address
-- STRAIGHT to the customer who ordered it. The link is stored on the
-- DRAFT (validated inside create_transfer_draft's locked section, so a
-- stale/fulfilled/unpaid line refuses before any money moves) and the
-- order line is stamped direct_fulfilled_at by finalizeTransfer — the
-- same statement that records the purchased label — so EVERY finalize
-- path (primary purchase, Check-Shippo-&-retry, recover-by-transaction,
-- Retry save) marks the customer's order, not just the happy path.
-- Tracking is surfaced on the order line by joining transfers through
-- this column (no tracking copy to drift).

ALTER TABLE transfers ADD COLUMN direct_order_item_id BIGINT REFERENCES order_items(id);

-- old 16-arg overload dropped; the function below is the only version
DROP FUNCTION create_transfer_draft(bigint, text, jsonb, jsonb, text, text, text, text, text, jsonb, boolean, jsonb, bigint, jsonb, text, text);

-- Full text — exactly what was executed live (verified installed via
-- pg_get_functiondef containing the p_direct_order_item_id branch).
-- Changes from 1786471800: 17th parameter p_direct_order_item_id; a
-- validation block refusing the draft unless the linked line is a live,
-- outstanding, MONEY-GATED (matched/over recon, no pending payments,
-- not held) direct-ship line whose product is among the transfer's
-- items and whose order has a ship-to address; the INSERT and audit row
-- carry the link.
...
BEGIN
  ...
  -- (same gates as the Fulfillment direct queue), have a ship-to
  -- address, and its product must actually be in this transfer
