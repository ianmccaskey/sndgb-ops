-- Review hardening, two fixes.
--
-- (1) RECLAIMED DRAFTS BECOME UNPURCHASABLE: reclaiming a stale direct
-- link now also stamps transfers.direct_link_reclaimed_at (new column),
-- and markTransferPurchaseStarted — the mandatory pre-POST claim for
-- EVERY fresh purchase path — refuses claims on a reclaimed draft, so
-- no NEW label can ever be bought against it (duplicate postage to the
-- same customer is unrepresentable). Recovery of an ALREADY-BOUGHT
-- label deliberately still works: the existing-transaction paths
-- (findTransactionByRate / recover-by-txn) call finalizeTransfer
-- WITHOUT a claim, and a reclaimed draft's link is already NULL, so
-- finalizing it records the label but never stamps any order line.
-- Verified-no-label deletion also still works.
--
-- (2) QUANTITY INVARIANT: the direct link now requires the transfer's
-- line for the customer's product to carry AT LEAST the outstanding
-- ordered quantity (COALESCE(qty_override, qty)) — a 1-unit transfer
-- can no longer mark a 3-unit line fulfilled. Over-shipping stays
-- allowed on purpose: the physical box may hold more than the order
-- (operator's call, the preview shows both numbers); under-shipping
-- must not silently complete the line — ship unlinked and mark
-- manually if a partial is really intended.

ALTER TABLE transfers ADD COLUMN direct_link_reclaimed_at TIMESTAMPTZ;
...
-- pg_get_functiondef containing direct_link_reclaimed_at and the
-- qty_override comparison).
