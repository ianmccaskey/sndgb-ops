...
LEFT JOIN (
  ...
  -- those kits (revenue at GB price − vendor cost − per-kit freight), so
  -- their net effect is always zero
  SELECT gbp.group_buy_id,
         SUM(a.qty * (gbp.gb_price_usd - gbp.unit_cost_usd - gbp.freight_usd)) AS at_cost_margin_usd
  FROM admin_adjustments a
  JOIN group_buy_products gbp ON gbp.id = a.group_buy_product_id
