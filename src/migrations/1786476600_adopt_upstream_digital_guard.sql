-- Codex post-push round 3: adopt_upstream_shipment lacked the digital
-- guards every other shipment-creating path carries — a product flipped
-- digital between dialog-open and confirm could receive a physical
-- shipment row. Mirrors the live create_manual_shipment: ordered
-- per-product prod_digital_ advisory locks (42007) after the order
-- lock, and NOT p.digital enforced inside the row-locked item
-- validation join. Same signature — CREATE OR REPLACE only.
