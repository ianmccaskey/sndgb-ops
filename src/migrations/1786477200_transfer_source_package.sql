-- Transfers record WHICH received package they parted out (Ian: "I
-- applied a transfer to a package and it's still showing available to
-- transfer"). transfers.source_package_id is set from the box picker;
-- the picker then EXCLUDES packages referenced by any finalized,
-- non-refund-voided transfer. Both create fns gain a trailing
-- p_source_package_id param (signature change: DROP + CREATE applied
-- live 2026-08-31): the id is stored only when it is a received package
-- whose address resolves to the from-origin group — anything else
-- records NULL rather than refusing the money path over a provenance
-- tag. Audit rows carry source_package_id.
ALTER TABLE transfers ADD COLUMN source_package_id bigint REFERENCES inbound_packages(id);

DROP FUNCTION IF EXISTS public.create_transfer_draft(bigint, text, jsonb, jsonb, text, text, text, text, text, jsonb, boolean, jsonb, bigint, jsonb, text, text, bigint, bigint);
DROP FUNCTION IF EXISTS public.create_manual_transfer(bigint, text, jsonb, text, text, text, jsonb, boolean, jsonb, bigint, jsonb, text, text, bigint, bigint);

CREATE OR REPLACE FUNCTION public.create_transfer_draft(p_from_address_id bigint, p_destination_label text, p_destination jsonb, p_parcel jsonb, p_carrier text, p_servicelevel text, p_rate_amount text, p_rate_currency text, p_shippo_rate_id text, p_items jsonb, p_allow_over_onhand boolean, p_expected_from jsonb, p_destination_id bigint, p_expected_destination jsonb, p_note text, p_actor text, p_direct_order_item_id bigint, p_group_buy_id bigint, p_source_package_id bigint)
...
BEGIN
  ...
  INSERT INTO transfers (from_address_id, from_label, from_address, destination_label, destination, parcel,
                         ...
                         purchase_started_at, direct_order_item_id, source_package_id)
  VALUES (v_addr.id, v_addr.label,
          ...
          NULLIF(TRIM(COALESCE(p_note, '')), ''), p_actor, now(), p_direct_order_item_id, v_src)
...
BEGIN
  ...
  INSERT INTO transfers (from_address_id, from_label, from_address, destination_label, destination, parcel,
                         ...
                         finalized_at, direct_order_item_id, direct_stamped_at, source_package_id)
  VALUES (v_addr.id, v_addr.label,
          jsonb_build_object('name', v_addr.name, 'street1', v_addr.street1, 'street2', v_addr.street2,
                             'city', v_addr.city, 'state', v_addr.state, 'zip', v_addr.zip,
                             'country', v_addr.country, 'phone', v_addr.phone, 'email', v_addr.email),
