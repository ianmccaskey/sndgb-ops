-- 1786468400 made order_items.qty fractional (split kits) but left the
-- audited manual-adjustment path integer: admin_adjustments.qty was INTEGER
-- and addAdjustment cast ::int, so an operator could not correct split-kit
-- demand by +/-0.5. Adjustments feed v_moq_progress.final_count directly, so
-- both quantity inputs must share NUMERIC(10,2).
--
-- v_moq_progress sums admin_adjustments.qty, so the same view chain as
-- 1786468400 must be dropped and recreated around the ALTER
...
ALTER TABLE admin_adjustments ALTER COLUMN qty TYPE NUMERIC(10,2);
